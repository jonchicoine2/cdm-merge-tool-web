 ...
 * --------------------
 * A small panel that surfaces high level merge metrics (matched / unmatched / duplicate, etc.).
 * Extracted from the ExcelImportPage to cut down that file size.
 */
const ComparisonStatsPanel: React.FC<Props> = ({ stats }) => {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 2, alignItems: "center", flexWrap: "wrap" }}>
        <Chip
          size="small"
          label={`${stats.matchedRecords.toLocaleString()} matched`}
          sx={{ backgroundColor: "#4caf50", color: "white", fontSize: "0.75rem" }}
        />
        <Chip
          size="small"
          label={`${stats.unmatchedRecords.toLocaleString()} unmatched`}
          sx={{ backgroundColor: "#f44336", color: "white", fontSize: "0.75rem" }}
        />
        <Chip
          size="small"
          label={`${stats.duplicateRecords.toLocaleString()} duplicates`}
          sx={{ backgroundColor: "#ff9800", color: "white", fontSize: "0.75rem" }}
        />
        <Chip
          size="small"
          label={`${stats.matchRate}% match rate`}
          sx={{ backgroundColor: "#2196f3", color: "white", fontSize: "0.75rem" }}
        />
        <Chip
          size="small"
          label={`${stats.columnsMatched} columns mapped`}
